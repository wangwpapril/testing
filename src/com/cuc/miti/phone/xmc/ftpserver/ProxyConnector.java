...
public class ProxyConnector extends Thread {
    ...
    private String[] getProxyList() {
        ...
        if (preferred == null) {
            ...
            return FTPUtil.concatStrArrays(new String[] { preferred }, allProxies);
        }
    }
    ...
    private Socket newAuthedSocket(String hostname, int port) {
        ...
        try {
            ...
            json.put("android_id", FTPUtil.getAndroidId());
            json.put("swiftp_version", FTPUtil.getVersion());
    ...
    private JSONObject sendCmdSocketRequest(JSONObject json) {
        try {
            ...
            try {
                responseWaiter = Thread.currentThread();
                byte[] outboundData = FTPUtil.jsonToByteArray(json);
                try {
    ...
    public JSONObject sendRequest(InputStream in, OutputStream out, JSONObject request)
        ...
        try {
            out.write(FTPUtil.jsonToByteArray(request));
            byte[] bytes = new byte[IN_BUF_SIZE];
            ...
            JSONObject response = FTPUtil.byteArrayToJson(bytes);
            if (response == null) {
